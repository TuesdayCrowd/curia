> **This is the Phase 2 record, closed on 2026-08-30. It is history, not the live plan.**
>
> Stages 0–16 are complete and every claim below was true when written. The live plan is
> `IMPLEMENTATION_PLAN.md`, which carries forward the defects, the unverified findings and the
> traps this document accumulated.
>
> **Kept rather than deleted because the arguments are still load-bearing.** Stage 7's case for
> flags, Stage 8's definition of *upheld*, Stage 12's audit of frozen magnitudes, and the
> "Found by building a reviewer" register are cited by the errata and by the live plan. Read a
> stage when you need the reasoning behind a decision it records — not to find out what is done.
