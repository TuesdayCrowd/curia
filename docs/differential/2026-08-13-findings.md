# Three-way differential findings — 2026-08-13

> **A frozen record: the analysis of the 2026-08-13 run, written when 15 classes were open.**
>
> Its verdicts on those 15 classes and its 18 proposed conformance vectors are history worth
> keeping — several were adopted, and E2/E4/E13/E14 in the errata came out of this. Its
> counts describe that run and nothing later: a run of the same command with the same
> seed on **2026-09-09** found **0 divergence classes across 22,520 compared lines**.
>
> The blockquote and paragraph that follow are about the session that produced this
> document — a tool refused to write the file, so it was pasted in — and not about Cūria.
> They are left as they were found. Its original title was "Task 7".
...
Synthesizes `2026-08-13-divergences.md` against three independent verification passes (RFC-8785/7493/ECMA-262 lens, Cūria-spec lens, reproducibility lens). Where the lenses agreed, the agreed verdict is stated; where they split, both readings are given, majority noted, minority not discarded.
...
admit-reject/too-many-members/
  input.json (9,141 bytes): {"k0":0,"k1":0,...,"k1024":0} -- 1,025 members,
    byte-identical to the reproducer in `2026-08-13-divergences.md`'s members-exceeded class.
