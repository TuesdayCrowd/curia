...
2. **Owner authentication.** Table 10 already has an "owner-auth only" cell for `agent`/`enroll`. But `owner_verified` is a **client-supplied boolean** — an open, confirmed defect in `IMPLEMENTATION_PLAN.md` — so grounding delegation on it would build a control on top of a value the party it constrains supplies. Rejected. *(Premise closed 2026-09-04 by Stage 1 / errata G5: owner verification is now an operator's attestation carrying `owner_id`. This rejection predates that and should be re-argued, not inherited.)*
