...
11. **A record that changes nothing is refused by name.** That means a record that would change
    neither the post's servability nor any flag's upheld state, and adjudicates no flag that no
    earlier record had adjudicated. R10.39 counts records, so no-op records would distort it. A
    dismissal of open flags is therefore permitted — it changes nothing served, but it records a
    review, which is R10.39's denominator.
